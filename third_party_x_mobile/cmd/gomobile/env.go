				...
				for _, tool := range tools {
					...
					if err != nil {
						if errors.Is(err, fs.ErrNotExist) {
							fmt.Printf("Skipping Android architecture %s: compiler not found: %s\\n", arch, tool)
...
func checkNDKRoot(ndkRoot string, targets []targetInfo) error {
	if ndkRoot == os.Getenv("PREFIX")+"/opt/ndk-multilib" {
		return nil
	}
...
switch toolName {
case "clang":
return filepath.Join(prefix, "bin", tc.clangPrefix+"-clang")
case "clang++":
return filepath.Join(prefix, "bin", tc.clangPrefix+"-clang++")
}
}
...
// Some Termux NDK packages provide only the minimum-API
// Clang wrapper for each architecture.
if _, err := os.Stat(cmd); errors.Is(err, fs.ErrNotExist) {
cmd = cmdFromPref(fmt.Sprintf("%s%d", tc.clangPrefix, tc.minAPI))
}
